...
void CNpc::OnDeathProcess(Unit *pKiller)
{
	...
	if (TO_NPC(this) != nullptr && pUser != nullptr)
	{
		...
		if (pNpc->isNPC())
		{
			if(m_tNpcType == NPC_CHAOS_STONE)
			{
				ChaosStone(pUser,5);
			}
			else if(m_tNpcType == NPC_MONUMENT_YNG)
			{
				MonumentNpc(pUser);
			}
			else if (m_tNpcType == NPC_BIFROST_MONUMENT)
			{
...
void CNpc::MonumentNpc(CUser *pUser)
{
	//CUser * pUser = TO_USER(pKiller);
	if (pUser == nullptr)
		return;
	...
	{
	g_pMain->SendNotice<DEATH_NOTICE>("",GetZoneID(),pUser->GetNation() == ELMORAD ? Nation::KARUS : Nation::ELMORAD);
	g_pMain->SendNotice<ANNOUNCEMENT_WHITE_CHAT>(string_format("- ## Human Irkýndan %s Ronark Land Monument'i Kesti ## -",pUser->GetName().c_str()).c_str(),GetZoneID(),pUser->GetNation());
	pUser->GiveItem(379154000);
	}
	if(pUser->GetNation() == KARUS)
	{
	g_pMain->SendNotice<DEATH_NOTICE>("",GetZoneID(),pUser->GetNation() == ELMORAD ? Nation::KARUS : Nation::ELMORAD);
	g_pMain->SendNotice<ANNOUNCEMENT_WHITE_CHAT>(string_format("- ## Karus Irkýndan %s Ronark Land Monument'i Kesti ## -",pUser->GetName().c_str()).c_str(),GetZoneID(),pUser->GetNation());
	pUser->GiveItem(379154000);
	}
	}
	else if (pUser->GetZoneID() == ZONE_ARDREAM)
	{
	if(pUser->GetNation() == ELMORAD)
	{
	g_pMain->SendNotice<DEATH_NOTICE>("",GetZoneID(),pUser->GetNation() == ELMORAD ? Nation::KARUS : Nation::ELMORAD);
	g_pMain->SendNotice<ANNOUNCEMENT_WHITE_CHAT>(string_format("- ## Human Irkýndan %s Andream Monument'i Kesti ## -",pUser->GetName().c_str()).c_str(),GetZoneID(),pUser->GetNation());
	}
	if(pUser->GetNation() == KARUS)
	{
	g_pMain->SendNotice<DEATH_NOTICE>("",GetZoneID(),pUser->GetNation() == ELMORAD ? Nation::KARUS : Nation::ELMORAD);
	g_pMain->SendNotice<ANNOUNCEMENT_WHITE_CHAT>(string_format("- ## Karus Irkýndan %s Andream Monument'i Kesti ## -",pUser->GetName().c_str()).c_str(),GetZoneID(),pUser->GetNation());
	}
	}
	else if (pUser->GetZoneID() == ZONE_RONARK_LAND_BASE)
	{
	if(pUser->GetNation() == ELMORAD)
	{
	g_pMain->SendNotice<DEATH_NOTICE>("",GetZoneID(),pUser->GetNation() == ELMORAD ? Nation::KARUS : Nation::ELMORAD);
	g_pMain->SendNotice<ANNOUNCEMENT_WHITE_CHAT>(string_format("- ## Human Irkýndan %s Ronark Land Base Monument'i Kesti ## -",pUser->GetName().c_str()).c_str(),GetZoneID(),pUser->GetNation());
	}
	if(pUser->GetNation() == KARUS)
	{
	g_pMain->SendNotice<DEATH_NOTICE>("",GetZoneID(),pUser->GetNation() == ELMORAD ? Nation::KARUS : Nation::ELMORAD);
	g_pMain->SendNotice<ANNOUNCEMENT_WHITE_CHAT>(string_format("- ## Karus Irkýndan %s Ronark Land Base Monument'i Kesti ## -",pUser->GetName().c_str()).c_str(),GetZoneID(),pUser->GetNation());
	}
	}
}